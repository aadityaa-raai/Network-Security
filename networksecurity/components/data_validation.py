...
class DataValidation:
    ...
    def validate_no_of_col(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Required no of cols= {number_of_col}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status=self.validate_no_of_col(dataframe=train_df)
            if not status:
                error_message="Train df doesnt contain all columns"
                logging.info(error_message)
            
            # status=self.validate_no_of_col(dataframe=test_df)
            # if not status:
            #     error_message=f"{error_message} Test df doesnt contain all columns"
